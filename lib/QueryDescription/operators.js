...
function between(left, right) {
  (0, _invariant["default"])(typeof left === 'number' && typeof right === 'number', 'Values passed to Q.between() are not numbers');
...
function like(value) {
  (0, _invariant["default"])(typeof value === 'string', 'Value passed to Q.like() is not a string');
  return {
    operator: 'like',
    right: {
      value: value
...
function notLike(value) {
  (0, _invariant["default"])(typeof value === 'string', 'Value passed to Q.notLike() is not a string');
  return {
    operator: 'notLike',
    right: {
      value: value
...
function sanitizeLikeString(value) {
  (0, _invariant["default"])(typeof value === 'string', 'Value passed to Q.sanitizeLikeString() is not a string');
  return value.replace(nonLikeSafeRegexp, '_');
}
function includes(value) {
  (0, _invariant["default"])(typeof value === 'string', 'Value passed to Q.includes() is not a string');
  return {
    operator: 'includes',
    right: {
      value: value
...
function column(name) {
  (0, _invariant["default"])(typeof name === 'string', 'Name passed to Q.column() is not a string');
  return {
...
function _valueOrComparison(arg) {
  if (arg === null || (0, _typeof2["default"])(arg) !== 'object') {
...
function unsafeSqlExpr(sql) {
  if (process.env.NODE_ENV !== 'production') {
    (0, _invariant["default"])(typeof sql === 'string', 'Value passed to Q.unsafeSqlExpr is not a string');
  }
  return {
    type: 'sql',
    expr: sql
...
function unsafeLokiExpr(expr) {
  if (process.env.NODE_ENV !== 'production') {
    (0, _invariant["default"])(expr && (0, _typeof2["default"])(expr) === 'object' && !Array.isArray(expr), 'Value passed to Q.unsafeLokiExpr is not an object');
  }
  return {
    type: 'loki',
    expr: expr
...
function sortBy(sortColumn) {
  ...
  (0, _invariant["default"])(sortOrder === 'asc' || sortOrder === 'desc', "Invalid sortOrder argument received in Q.sortBy (valid: asc, desc)");
  return {
    type: 'sortBy',
...
function take(count) {
  (0, _invariant["default"])(typeof count === 'number', 'Value passed to Q.take() is not a number');
  return {
    type: 'take',
    count: count
  };
...
function skip(count) {
  (0, _invariant["default"])(typeof count === 'number', 'Value passed to Q.skip() is not a number');
  return {
    type: 'skip',
    count: count
  };
...
var _on = exports.on = function on(table, leftOrClauseOrList, valueOrComparison) {
...
function unsafeSqlQuery(sql) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    (0, _invariant["default"])(typeof sql === 'string', 'Value passed to Q.unsafeSqlQuery is not a string');
